...
export const FinancialDictionaryView: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h2 className="text-xl font-bold text-slate-100">Financial Lexicon & Word Dictionary</h2>
        ...
        <div className="flex items-center gap-3 text-xs font-mono">
          <div className="px-3 py-1.5 bg-emerald-950/60 border border-emerald-800/60 text-emerald-300 rounded-xl flex items-center gap-1.5 font-bold">
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
            <span>{allPositive.length} Positive Terms</span>
          </div>
          <div className="px-3 py-1.5 bg-rose-950/60 border border-rose-800/60 text-rose-300 rounded-xl flex items-center gap-1.5 font-bold">
      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4 text-xs text-slate-300 space-y-2 font-sans">
        <div className="flex items-center gap-2 font-bold text-cyan-400 font-mono text-xs uppercase tracking-wider">
          <Info className="w-4 h-4 text-cyan-400" /> Multi-Layered Sentiment Analysis Context
        </div>
        <p className="text-slate-400 leading-relaxed">
          The dictionary lexicon provides rule-based keyword weights for financial headline parsing. Note that keyword matches alone do not determine direction. The final sentiment combines <strong className="text-slate-200">lexicon rule matches</strong>, <strong className="text-slate-200">VADER sentiment intensity</strong>, and <strong className="text-slate-200">headline context</strong> before outputting a research score.
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
        <h3 className="text-sm font-bold text-slate-200 flex items-center gap-2 font-mono uppercase tracking-wider">
          <Plus className="w-4 h-4 text-cyan-400" /> Add Custom Word to Sentiment Engine
        </h3>
        ...
        <form onSubmit={handleAddWord} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          <div className="relative flex-1">
            <input
              ...
              className="w-full px-4 py-2.5 bg-slate-950 border border-slate-700/80 rounded-xl text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-cyan-500 font-medium"
            />
          </div>
          ...
          <select
            ...
            className="px-3 py-2.5 bg-slate-950 border border-slate-700/80 rounded-xl text-xs font-bold font-mono text-slate-200 focus:outline-none focus:border-cyan-500 cursor-pointer"
          >
          ...
          <button
            type="submit"
            className="px-5 py-2.5 bg-cyan-500 hover:bg-cyan-400 active:bg-cyan-600 text-slate-950 font-bold font-mono text-xs rounded-xl transition-all cursor-pointer shadow-md shadow-cyan-500/10 flex items-center justify-center gap-1.5 whitespace-nowrap"
          >
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 bg-slate-900/60 p-4 rounded-2xl border border-slate-800">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-slate-950 border border-slate-700/80 rounded-xl text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-cyan-500"
          />
        </div>

        <div className="flex items-center gap-1 text-xs font-mono">
          <span className="text-slate-400 mr-2 flex items-center gap-1">
            <Filter className="w-3.5 h-3.5 text-cyan-400" /> Filter View:
          </span>
          <button
            onClick={() => setCategoryFilter('ALL')}
            className={`px-3 py-1.5 rounded-lg border transition-all cursor-pointer ${
              categoryFilter === 'ALL'
                ? 'bg-slate-800 text-cyan-400 font-bold border-cyan-500'
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg border transition-all cursor-pointer ${
              categoryFilter === 'POSITIVE'
                ? 'bg-emerald-950 text-emerald-300 font-bold border-emerald-500'
                : 'bg-slate-950 text-slate-400 border-slate-800 hover:text-slate-200'
            }`}
          >
            Positive Only
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg border transition-all cursor-pointer ${
              categoryFilter === 'NEGATIVE'
                ? 'bg-rose-950 text-rose-300 font-bold border-rose-500'
                : 'bg-slate-950 text-slate-400 border-slate-800 hover:text-slate-200'
            }`}
          >
            Negative Only
      ...
      {isLoading ? (
        <div className="p-12 text-center text-slate-400 font-mono text-sm bg-slate-900 border border-slate-800 rounded-2xl">
            ...
            <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 space-y-4 shadow-xl">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-base font-bold text-slate-100 font-mono">POSITIVE WORDS</h3>
                </div>
                <span className="text-xs font-mono px-2.5 py-1 bg-emerald-950/80 text-emerald-300 border border-emerald-800/80 rounded-lg font-bold">
              ...
              <div className="flex flex-wrap gap-2 max-h-[450px] overflow-y-auto pr-1">
                {displayedPositive.map((item) => (
                  ...
                  >
                    ...
                    {item.isCustom ? (
                      <span className="text-[9px] uppercase px-1.5 py-0.2 bg-emerald-500 text-slate-950 rounded font-bold">
                        User Custom
                      </span>
                    ) : (
                ...
                {displayedPositive.length === 0 && (
                  <div className="text-xs text-slate-500 italic py-4">No positive words matching filter.</div>
                )}
              </div>
            </div>
          )}
            ...
            <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 space-y-4 shadow-xl">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-base font-bold text-slate-100 font-mono">NEGATIVE WORDS</h3>
                </div>
                <span className="text-xs font-mono px-2.5 py-1 bg-rose-950/80 text-rose-300 border border-rose-800/80 rounded-lg font-bold">
              ...
              <div className="flex flex-wrap gap-2 max-h-[450px] overflow-y-auto pr-1">
                {displayedNegative.map((item) => (
                  ...
                  >
                    ...
                    {item.isCustom ? (
                      <span className="text-[9px] uppercase px-1.5 py-0.2 bg-rose-500 text-slate-950 rounded font-bold">
                        User Custom
                      </span>
                    ) : (
                      <span className="text-[9px] uppercase px-1.5 py-0.2 bg-slate-800 text-slate-400 rounded font-bold">
                ...
                {displayedNegative.length === 0 && (
                  <div className="text-xs text-slate-500 italic py-4">No negative words matching filter.</div>
                )}
              </div>
            </div>
          )}
